...
class manageAllBookingsController extends Controller
{
    ...
    {
        return view('bookings.manageBookings');
    }

    public function getAllBookingList()
    {
        try {
            $res = $this->apiGet('Booking/GET_AllBookings')->json();
    ...
    {
        try {
            $res = $this->apiGet('Booking/GET_PendingBookings')->json();
            return response()->json(['data' => ($res['Success'] ?? false) ? ($res['Data'] ?? []) : []]);
        } catch (\Exception $e) {
            return response()->json(['data' => [], 'error' => $e->getMessage()]);
        }
